# ♻️ Greenery (data preprocessing & backend)
...
- `유리병`과 `나무`, `스티로폼`의 클래스 명이 `유리병류`, `나무류`, `스티로폼류`로 구분되어 있어 폴더명을 바꿔주었음.
- `플라스틱류`의 `장남감`이 오타로 처리되어 있지만, 라벨링 데이터에도 `장남감`으로 처리되어 있어서 수정하지 못함.
